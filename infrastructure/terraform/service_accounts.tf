...
resource "google_service_account" "proxy" {
  ...
  description  = "Service account for cf-proxy. Publishes events to Pub/Sub and invokes internal services."
}

resource "google_service_account" "frontend" {
  account_id   = "frontend-svc"
  display_name = "frontend service account"
  description  = "Service account for pixelhub-frontend. Reads DISCORD_CLIENT_SECRET for OAuth token exchange. No project-level IAM bindings by design."
}
